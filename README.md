...
Cortex lets you organize your workspace files by **contexts**, **tags**, and **file types** without moving files or creating folders. Think of it like Windows Media Player organizing MP3s by Artist, Album, and Genre — but for code, documents, and all your work files.

## Core Principle

- **Files stay where they are** - No duplication, no moving
- **Multiple virtual views** - See the same files organized different ways
- **Files belong to multiple contexts** - A file can be tagged "urgent" and belong to "project-alpha" and "client-acme" simultaneously
...
- **By Context** - Group files by projects, clients, cases, or any semantic container
...
- `Cortex: Assign context to current file` - Add file to a context
...
2. **Add contexts** - Right-click a file → "Cortex: Assign context to current file"
...
│   │   └── MetadataStore.ts      # SQLite persistence (tags, contexts)
│   ├── views/
